    ...
    public class ShooterProvider : ISubtitleProvider, IHasOrder
    {
        ...
        private async Task<IEnumerable<RemoteSubtitleInfo>> SearchSubtitlesAsync(SubtitleSearchRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode && response.Content.Headers.ContentType?.MediaType?.Equals("application/json", StringComparison.OrdinalIgnoreCase) == true)
        ...
        private static void ValidateSubtitleData(byte[] data, string format, string mediaType)
        {
            ...
            var valid = format == SRT ? sample.Contains("-->", StringComparison.Ordinal) :
        ...
        public static async Task<string> ComputeFileHashAsync(FileInfo fileInfo, CancellationToken cancellationToken)
        {
            ...
            using (FileStream fs = new FileStream(fileInfo.FullName, FileMode.Open, FileAccess.Read, FileShare.Read, 4096, FileOptions.Asynchronous))
            {
                ...
                for (int i = 0; i < 4; ++i)
                {
                    ...
                    await fs.ReadExactlyAsync(bBuf, 0, 4 * 1024, cancellationToken);
